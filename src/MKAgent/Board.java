...
public class Board extends Observable implements Cloneable {
    ...
    public Board(int holes, int seeds) throws IllegalArgumentException {
        ...
        if (seeds < 0)
            throw new IllegalArgumentException("There has to be a non-negative number of seeds, but " + seeds
                                               + " were requested.");
    ...
    public int getSeeds(Side side, int hole) throws IllegalArgumentException {
        if (hole < 1 || hole > holes)
            throw new IllegalArgumentException("Hole number must be between 1 and " + (board[NORTH_ROW].length - 1)
                                               + " but was " + hole + ".");
    ...
    public String toString() {
        ...
        boardString.append(board[NORTH_ROW][0]).append("  --");
        for (int i = holes; i >= 1; i--)
            boardString.append("  ").append(board[NORTH_ROW][i]);
        boardString.append("\n");
        for (int i = 1; i <= holes; i++)
            boardString.append(board[SOUTH_ROW][i]).append("  ");
        boardString.append("--  ").append(board[SOUTH_ROW][0]).append("\n");
